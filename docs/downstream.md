...
>
> ⚠️ sigil / reeve 仍停在 `2b03e8d`，落后 `2a32261` 两处：`parse_profiles`（多条打包）与新增 6 家服务商。
> 升级时要补这 6 家的 `providerTemplate.*` 翻译（sigil `check-preset-i18n.py`、reeve 守卫 `ai_profile_preset_i18n_keys_exist` 会报缺）。
