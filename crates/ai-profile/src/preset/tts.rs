...
pub(super) const TTS_PRESETS: &[ProviderPreset] = &[
    ...
    ProviderPreset {
        ...
        group_key: GROUP_CHINA.0,
        group_label: GROUP_CHINA.1,
        label_key: "providerTemplate.ipsunionTts.label",
        label: "中宇AI智行 配音（GLM-TTS）",
        hint_key: Some("providerTemplate.ipsunionTts.hint"),
        hint: Some("glm-tts 只出 wav（不支持 mp3），音色为固定枚举、不可克隆；与对话同一个密钥"),
        base_url: Some("https://api.ipsunion.com/v1"),
        model: "glm-tts",
        models: &[ModelOption::plain("glm-tts")],
        protocol: Protocol::OpenAiCompatible,
